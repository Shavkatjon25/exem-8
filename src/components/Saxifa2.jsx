...
function Saxifa2() {
   ...
   async function Data(){
    try{
        ...
        const d=await fetch(`https://api.spotify.com/v1/playlists/${playlistId.id}`,
        {
            headers:{
    ...
    setMal(dj)
    setMal34(dj.tracks.items.splice(0, 15));
    }
    catch(err){
        console.log(err);
    ...
    console.log(mal34);
  return (
    <div className='pt-5 px-[41px] max-w-[1072px] flex-1 w-full box-border pb-10 sahifa2'>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M13.0702 11.302C13.4745 11.7064 13.458 12.3669 13.034 12.7506L2.33908 22.4269C1.82715 22.8901 1.03669 22.8506 0.573517 22.3386C0.110348 21.8267 0.149872 21.0362 0.661796 20.5731L10.1874 11.9547L0.616553 2.38388C0.128398 1.89573 0.128398 1.10427 0.616553 0.616116C1.10471 0.127961 1.89617 0.127961 2.38432 0.616117L13.0702 11.302Z" fill="white"/>
        ...
        <div className='mt-12 flex gap-8 items-end'>
            <div>
                {mal ? <img src={mal.images[0].url} alt="" className='w-[297px] h-[297px]' /> : <Skeleton className="w-[297px] h-[297px] m-0" baseColor='#ffffff1c'></Skeleton>}
            </div>
            <div>
                <h3 className='text-[1rem] font-medium text-white fnt'>PUBLIC PLAYLIST</h3>
                <h2 className='text-[112px] font-[900] text-white'>{mal ? mal.name: <Skeleton className="w-[200px] h-[112px] m-0" baseColor='#ffffff1c'></Skeleton>}</h2>
                <p className='fnt font-[450] text-[18px] text-[#b3b3b3]'>{mal ? mal.description : <Skeleton className="w-[150px] h-[18px] m-0" baseColor='#ffffff1c'></Skeleton>}</p>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M34.839 28.9289L26.884 36.8839C26.3959 37.372 25.6044 37.372 25.1162 36.8839L17.1613 28.9289C16.6731 28.4408 16.6731 27.6493 17.1613 27.1612C17.6494 26.673 18.4409 26.673 18.9291 27.1612L24.7501 32.9822L24.7501 17C24.7501 16.3096 25.3098 15.75 26.0001 15.75C26.6905 15.75 27.2501 16.3096 27.2501 17L27.2501 32.9822L33.0712 27.1612C33.5594 26.673 34.3508 26.673 34.839 27.1612C35.3271 27.6493 35.3271 28.4408 34.839 28.9289Z" fill="white"/>
        ...
        {   mal34 ? 
            mal34.map((a, index)=>{
                ...
                return(
                    <div key={index} className='flex items-center pl-5 pr-[63px] mt-[22px] justify-between'onClick={()=>dispach(Omborchi.actions.pleylist(a))} >
                    <div className='flex'>
                ...
                return(
                    <div className='flex items-center pl-5 pr-[63px] mt-[22px] justify-between' key={a} >
                    <div className='flex gap-2'>
                    <p className='font-[450] text-[22px] text-[#b3b3b3]'>{a}</p>
                        <Skeleton className="w-[52px] h-[52px] m-0" baseColor='#ffffff1c'></Skeleton>
                        <div className='flex flex-col gap-1'>
                            <h4 className='font-[450]  text-[20px] text-[#fff] leading-[25px] pt-1'><Skeleton className="w-[82px] h-[18px] m-0" baseColor='#ffffff1c'></Skeleton></h4>
                            <p className='fnt font-[450] text-[18px] text-[#b3b3b3]'><Skeleton className="w-[82px] h-[16px] m-0" baseColor='#ffffff1c'></Skeleton></p>
                        </div>
                    </div>
                    <div className='max-w-[400px] flex-1 justify-between flex'>
                    <h4 className='fnt  font-[450] text-[18px] text-[#b3b3b3]'><Skeleton className="w-[82px] h-[18px] m-0" baseColor='#ffffff1c'></Skeleton></h4>
                        <p className='fnt font-[450] text-[20px] text-white'><Skeleton className="w-[60px] h-[18px] m-0" baseColor='#ffffff1c'></Skeleton></p>
                    </div>
                    </div>
                )
            })
